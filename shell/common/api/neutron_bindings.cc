...
NeutronBindings::NeutronBindings(uv_loop_t* loop) {
  uv_async_init(loop, call_next_tick_async_.get(), OnCallNextTick);
  call_next_tick_async_.get()->data = this;
  metrics_ = base::ProcessMetrics::CreateCurrentProcessMetrics();
}

NeutronBindings::~NeutronBindings() = default;

// static
void NeutronBindings::BindProcess(v8::Isolate* isolate,
  ...
  process->SetMethod("getCPUUsage",
                     base::BindRepeating(&NeutronBindings::GetCPUUsage,
...
void NeutronBindings::BindTo(v8::Isolate* isolate,
  ...
  dict.SetMethod("activateUvLoop",
                 base::BindRepeating(&NeutronBindings::ActivateUVLoop,
                                     base::Unretained(this)));
}

void NeutronBindings::EnvironmentDestroyed(node::Environment* env) {
  std::erase(pending_next_ticks_, env);
}

void NeutronBindings::ActivateUVLoop(v8::Isolate* isolate) {
...
// static
void NeutronBindings::OnCallNextTick(uv_async_t* handle) {
  auto* self = static_cast<NeutronBindings*>(handle->data);
...
// static
void NeutronBindings::Crash() {
  volatile int* zero = nullptr;
  *zero = 0;
}

// static
void NeutronBindings::Hang() {
  for (;;)
    base::PlatformThread::Sleep(base::Seconds(1));
}

// static
v8::Local<v8::Value> NeutronBindings::GetHeapStatistics(v8::Isolate* isolate) {
...
// static
v8::Local<v8::Value> NeutronBindings::GetCreationTime(v8::Isolate* isolate) {
...
// static
v8::Local<v8::Value> NeutronBindings::GetSystemMemoryInfo(
...
// static
v8::Local<v8::Promise> NeutronBindings::GetProcessMemoryInfo(
    v8::Isolate* isolate) {
  CHECK(neutron::IsBrowserProcess());
...
// static
v8::Local<v8::Value> NeutronBindings::GetBlinkMemoryInfo(
    v8::Isolate* isolate) {
...
// static
void NeutronBindings::DidReceiveMemoryDump(
...
// static
v8::Local<v8::Value> NeutronBindings::GetCPUUsage(
    base::ProcessMetrics* metrics,
    v8::Isolate* isolate) {
  auto dict = gin_helper::Dictionary::CreateEmpty(isolate);
...
// static
bool NeutronBindings::TakeHeapSnapshot(v8::Isolate* isolate,
  ...
  return neutron::TakeHeapSnapshot(isolate, &file);
}
