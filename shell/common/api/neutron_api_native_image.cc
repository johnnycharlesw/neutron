...
NativeImage::NativeImage(v8::Isolate* isolate, const base::FilePath& hicon_path)
  ...
  neutron::util::ReadImageSkiaFromICO(&image_skia, GetHICON(256));
...
v8::Local<v8::Value> NativeImage::ToPNG(gin::Arguments* args) {
  ...
  if (scale_factor == 1.0f) {
    ...
    if (!png_span.empty())
      return neutron::Buffer::Copy(isolate, png_span).ToLocalChecked();
  }
  ...
  return neutron::Buffer::Copy(isolate, *encoded).ToLocalChecked();
}
...
v8::Local<v8::Value> NativeImage::ToJPEG(v8::Isolate* isolate, int quality) {
  ...
  if (!encoded_image)
    return NewEmptyBuffer(isolate);
  return neutron::Buffer::Copy(isolate, *encoded_image).ToLocalChecked();
}
...
v8::Local<v8::Value> NativeImage::GetNativeHandle(
  ...
  return neutron::Buffer::Copy(isolate, base::byte_span_from_ref(ptr))
      .ToLocalChecked();
#else
...
void NativeImage::AddRepresentation(const gin_helper::Dictionary& options) {
  ...
  if (options.Get("buffer", &buffer) && node::Buffer::HasInstance(buffer)) {
    skia_rep_added = neutron::util::AddImageSkiaRepFromBuffer(
        &image_skia, neutron::Buffer::as_byte_span(buffer), width, height,
        scale_factor);
  } else if (options.Get("dataURL", &url)) {
    std::string mime_type, charset, data;
    if (net::DataURL::Parse(url, &mime_type, &charset, &data)) {
      if (mime_type == "image/png") {
        skia_rep_added = neutron::util::AddImageSkiaRepFromPNG(
            &image_skia, base::as_byte_span(data), scale_factor);
      } else if (mime_type == "image/jpeg") {
        skia_rep_added = neutron::util::AddImageSkiaRepFromJPEG(
            &image_skia, base::as_byte_span(data), scale_factor);
      }
    }
  }
...
gin_helper::Handle<NativeImage> NativeImage::CreateFromPNG(
  ...
  gfx::ImageSkia image_skia;
  neutron::util::AddImageSkiaRepFromPNG(&image_skia, data, 1.0);
...
gin_helper::Handle<NativeImage> NativeImage::CreateFromJPEG(
  ...
  gfx::ImageSkia image_skia;
  neutron::util::AddImageSkiaRepFromJPEG(&image_skia, buffer, 1.0);
  return Create(isolate, gfx::Image(image_skia));
}
...
#endif
  gfx::ImageSkia image_skia;
  neutron::util::PopulateImageSkiaRepsFromPath(&image_skia, image_path);
  gfx::Image image(image_skia);
...
gin_helper::Handle<NativeImage> NativeImage::CreateFromBitmap(
  ...
  const auto buffer_data = neutron::Buffer::as_byte_span(buffer);
...
gin_helper::Handle<NativeImage> NativeImage::CreateFromBuffer(
  ...
  gfx::ImageSkia image_skia;
  neutron::util::AddImageSkiaRepFromBuffer(
      &image_skia, neutron::Buffer::as_byte_span(buffer), width, height,
      scale_factor);
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_common_native_image, Initialize)
