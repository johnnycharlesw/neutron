...
class NeutronBindings {
 public:
  explicit NeutronBindings(uv_loop_t* loop);
  virtual ~NeutronBindings();

  // disable copy
  NeutronBindings(const NeutronBindings&) = delete;
  NeutronBindings& operator=(const NeutronBindings&) = delete;

  // Add process._linkedBinding function, which behaves like process.binding
  // but load native code from Neutron instead.
