...
#include "shell/browser/neutron_download_manager_delegate.h"
...
#include "shell/browser/extensions/neutron_browser_context_keyed_service_factories.h"
...
[[nodiscard]] auto& ContextMap() {
  static base::NoDestructor<
      std::map<PartitionKey, std::unique_ptr<NeutronBrowserContext>>>
      map;
  return *map;
}
...
// static
std::vector<NeutronBrowserContext*> NeutronBrowserContext::BrowserContexts() {
  return base::ToVector(ContextMap(),
                        [](auto& iter) { return iter.second.get(); });
}

bool NeutronBrowserContext::IsValidContext(const void* context) {
...
// static
void NeutronBrowserContext::DestroyAllContexts() {
  auto& map = ContextMap();
...
NeutronBrowserContext::NeutronBrowserContext(
  ...
  if (!in_memory_) {
    ...
    auto* extension_system = static_cast<extensions::NeutronExtensionSystem*>(
...
void NeutronBrowserContext::InitPrefs() {
  ...
  NeutronAccessibilityUIMessageHandler::RegisterPrefs(registry.get());
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
  if (!in_memory_)
  ...
  registry->RegisterStringPref(neutron::kNeutronGlobalShortcutsUuid,
                               std::string());
}

void NeutronBrowserContext::SetUserAgent(const std::string& user_agent) {
  user_agent_ = user_agent;
}

base::FilePath NeutronBrowserContext::GetPath() const {
  return path_;
}

bool NeutronBrowserContext::IsOffTheRecord() {
  return in_memory_;
}

std::string NeutronBrowserContext::GetMediaDeviceIDSalt() {
...
NeutronBrowserContext::CreateZoomLevelDelegate(
...
NeutronBrowserContext::GetDownloadManagerDelegate() {
  if (!download_manager_delegate_) {
    download_manager_delegate_ =
        std::make_unique<NeutronDownloadManagerDelegate>(GetDownloadManager());
  }
  return download_manager_delegate_.get();
}

content::BrowserPluginGuestManager* NeutronBrowserContext::GetGuestManager() {
  if (!guest_manager_)
...
NeutronBrowserContext::GetPlatformNotificationService() {
  return NeutronBrowserClient::Get()->GetPlatformNotificationService();
}

content::PermissionControllerDelegate*
NeutronBrowserContext::GetPermissionControllerDelegate() {
  if (!permission_manager_)
    permission_manager_ = std::make_unique<NeutronPermissionManager>();
  return permission_manager_.get();
}

storage::SpecialStoragePolicy*
NeutronBrowserContext::GetSpecialStoragePolicy() {
  return storage_policy_.get();
}

std::string NeutronBrowserContext::GetUserAgent() const {
  return user_agent_.value_or(NeutronBrowserClient::Get()->GetUserAgent());
}

content::PreconnectManager* NeutronBrowserContext::GetPreconnectManager() {
  if (!preconnect_manager_) {
    preconnect_manager_delegate_ =
        std::make_unique<NeutronPreconnectManagerDelegate>();
...
NeutronBrowserContext::GetURLLoaderFactory() {
  ...
  mojo::PendingRemote<network::mojom::TrustedURLLoaderHeaderClient>
      header_client;
  static_cast<content::ContentBrowserClient*>(NeutronBrowserClient::Get())
...
NeutronBrowserContext::GetPushMessagingService() {
  return nullptr;
}

content::SSLHostStateDelegate*
NeutronBrowserContext::GetSSLHostStateDelegate() {
  return nullptr;
}

content::BackgroundFetchDelegate*
NeutronBrowserContext::GetBackgroundFetchDelegate() {
  return nullptr;
}

content::BackgroundSyncController*
NeutronBrowserContext::GetBackgroundSyncController() {
  return nullptr;
}

content::BrowsingDataRemoverDelegate*
NeutronBrowserContext::GetBrowsingDataRemoverDelegate() {
  return nullptr;
}

content::ClientHintsControllerDelegate*
NeutronBrowserContext::GetClientHintsControllerDelegate() {
  return nullptr;
}

content::StorageNotificationService*
NeutronBrowserContext::GetStorageNotificationService() {
  return nullptr;
}

content::ReduceAcceptLanguageControllerDelegate*
NeutronBrowserContext::GetReduceAcceptLanguageControllerDelegate() {
...
NeutronBrowserContext::GetFileSystemAccessPermissionContext() {
  return FileSystemAccessPermissionContextFactory::GetForBrowserContext(this);
}

ResolveProxyHelper* NeutronBrowserContext::GetResolveProxyHelper() {
...
network::mojom::SSLConfigPtr NeutronBrowserContext::GetSSLConfig() {
  return ssl_config_.Clone();
}

void NeutronBrowserContext::SetSSLConfig(network::mojom::SSLConfigPtr config) {
...
bool NeutronBrowserContext::ChooseDisplayMediaDevice(
    const content::MediaStreamRequest& request,
    content::MediaResponseCallback callback) {
...
void NeutronBrowserContext::GrantDevicePermission(
...
void NeutronBrowserContext::RevokeDevicePermission(
    const url::Origin& origin,
    const base::Value& device,
    const blink::PermissionType permission_type) {
...
bool NeutronBrowserContext::CheckDevicePermission(
    const url::Origin& origin,
    const base::Value& device,
    blink::PermissionType permission_type) {
  const auto& current_devices_it = granted_devices_.find(permission_type);
...
// static
NeutronBrowserContext* NeutronBrowserContext::From(
    const std::string& partition,
    bool in_memory,
    base::Value::Dict options) {
  auto& context = ContextMap()[PartitionKey(partition, in_memory)];
  if (!context) {
    context.reset(new NeutronBrowserContext{std::cref(partition), in_memory,
                                             std::move(options)});
  }
  return context.get();
}
...
// static
NeutronBrowserContext* NeutronBrowserContext::GetDefaultBrowserContext(
    base::Value::Dict options) {
  return NeutronBrowserContext::From("", false, std::move(options));
}

NeutronBrowserContext* NeutronBrowserContext::FromPath(
    const base::FilePath& path,
    base::Value::Dict options) {
  auto& context = ContextMap()[PartitionKey(path)];
  if (!context) {
    context.reset(
        new NeutronBrowserContext{std::cref(path), false, std::move(options)});
  }
  return context.get();
}
