...
namespace neutron {
NeutronWebContentsUtilityHandlerImpl::NeutronWebContentsUtilityHandlerImpl(
    content::RenderFrameHost* frame_host,
    mojo::PendingAssociatedReceiver<mojom::NeutronWebContentsUtility> receiver)
  ...
  receiver_.set_disconnect_handler(base::BindOnce(
      &NeutronWebContentsUtilityHandlerImpl::OnConnectionError, GetWeakPtr()));
}

NeutronWebContentsUtilityHandlerImpl::
    ~NeutronWebContentsUtilityHandlerImpl() = default;

void NeutronWebContentsUtilityHandlerImpl::RenderFrameDeleted(
...
void NeutronWebContentsUtilityHandlerImpl::OnConnectionError() {
  delete this;
}

void NeutronWebContentsUtilityHandlerImpl::OnFirstNonEmptyLayout() {
...
void NeutronWebContentsUtilityHandlerImpl::SetTemporaryZoomLevel(
    double level) {
  api::WebContents* api_web_contents = api::WebContents::From(web_contents());
  if (api_web_contents) {
...
void NeutronWebContentsUtilityHandlerImpl::CanAccessClipboardDeprecated(
...
NeutronWebContentsUtilityHandlerImpl::GetRenderFrameHost() {
  return content::RenderFrameHost::FromFrameToken(render_frame_host_token_);
}

// static
void NeutronWebContentsUtilityHandlerImpl::Create(
    content::RenderFrameHost* frame_host,
    mojo::PendingAssociatedReceiver<mojom::NeutronWebContentsUtility>
        receiver) {
  new NeutronWebContentsUtilityHandlerImpl(frame_host, std::move(receiver));
}
}  // namespace neutron
