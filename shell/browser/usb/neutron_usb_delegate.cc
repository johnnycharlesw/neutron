...
neutron::UsbChooserContext* GetChooserContext(
    content::BrowserContext* browser_context) {
  return neutron::UsbChooserContextFactory::GetForBrowserContext(
      browser_context);
}
...
class NeutronUsbDelegate::ContextObservation
    : private UsbChooserContext::DeviceObserver {
 public:
  ContextObservation(NeutronUsbDelegate* parent,
                     content::BrowserContext* browser_context)
 ...
 private:
  ...
  const raw_ptr<NeutronUsbDelegate> parent_;
...
NeutronUsbDelegate::NeutronUsbDelegate() = default;

NeutronUsbDelegate::~NeutronUsbDelegate() = default;

void NeutronUsbDelegate::AdjustProtectedInterfaceClasses(
    content::BrowserContext* browser_context,
    const url::Origin& origin,
...
std::unique_ptr<content::UsbChooser> NeutronUsbDelegate::RunChooser(
    content::RenderFrameHost& frame,
...
bool NeutronUsbDelegate::CanRequestDevicePermission(
    content::BrowserContext* browser_context,
    const url::Origin& origin) {
  if (!browser_context)
    return false;
...
void NeutronUsbDelegate::RevokeDevicePermissionWebInitiated(
    content::BrowserContext* browser_context,
    const url::Origin& origin,
...
const device::mojom::UsbDeviceInfo* NeutronUsbDelegate::GetDeviceInfo(
    content::BrowserContext* browser_context,
...
bool NeutronUsbDelegate::HasDevicePermission(
    content::BrowserContext* browser_context,
    content::RenderFrameHost* frame,
    const url::Origin& origin,
    const device::mojom::UsbDeviceInfo& device_info) {
...
void NeutronUsbDelegate::GetDevices(
    content::BrowserContext* browser_context,
...
void NeutronUsbDelegate::GetDevice(
    content::BrowserContext* browser_context,
    const std::string& guid,
...
void NeutronUsbDelegate::AddObserver(content::BrowserContext* browser_context,
                                      Observer* observer) {
  if (!browser_context)
    return;
...
void NeutronUsbDelegate::RemoveObserver(
    content::BrowserContext* browser_context,
    Observer* observer) {
  if (!browser_context)
    return;
...
NeutronUsbDelegate::ContextObservation*
NeutronUsbDelegate::GetContextObserver(
    content::BrowserContext* browser_context) {
...
bool NeutronUsbDelegate::IsServiceWorkerAllowedForOrigin(
    const url::Origin& origin) {
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
...
void NeutronUsbDelegate::DeleteControllerForFrame(
    content::RenderFrameHost* render_frame_host) {
  controller_map_.erase(render_frame_host);
}

bool NeutronUsbDelegate::PageMayUseUsb(content::Page& page) {
