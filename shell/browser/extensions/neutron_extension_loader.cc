...
NeutronExtensionLoader::NeutronExtensionLoader(
...
NeutronExtensionLoader::~NeutronExtensionLoader() = default;

void NeutronExtensionLoader::LoadExtension(
    const base::FilePath& extension_dir,
    int load_flags,
    base::OnceCallback<void(const Extension*, const std::string&)> cb) {
  GetExtensionFileTaskRunner()->PostTaskAndReplyWithResult(
      FROM_HERE, base::BindOnce(&LoadUnpacked, extension_dir, load_flags),
      base::BindOnce(&NeutronExtensionLoader::FinishExtensionLoad,
                     weak_factory_.GetWeakPtr(), std::move(cb)));
}

void NeutronExtensionLoader::ReloadExtension(const ExtensionId& extension_id) {
...
void NeutronExtensionLoader::UnloadExtension(
    const ExtensionId& extension_id,
    extensions::UnloadedExtensionReason reason) {
  extension_registrar_->RemoveExtension(extension_id, reason);
}

void NeutronExtensionLoader::FinishExtensionLoad(
    base::OnceCallback<void(const Extension*, const std::string&)> cb,
...
void NeutronExtensionLoader::FinishExtensionReload(
...
void NeutronExtensionLoader::PreAddExtension(const Extension* extension,
...
void NeutronExtensionLoader::PostActivateExtension(
    scoped_refptr<const Extension> extension) {}

void NeutronExtensionLoader::PostDeactivateExtension(
    scoped_refptr<const Extension> extension) {}

void NeutronExtensionLoader::PreUninstallExtension(
    scoped_refptr<const Extension> extension) {}

void NeutronExtensionLoader::PostUninstallExtension(
    scoped_refptr<const Extension> extension,
    base::OnceClosure done_callback) {}

void NeutronExtensionLoader::DoLoadExtensionForReload(
    const ExtensionId& extension_id,
  ...
  GetExtensionFileTaskRunner()->PostTaskAndReplyWithResult(
      ...
      base::BindOnce(&NeutronExtensionLoader::FinishExtensionReload,
                     weak_factory_.GetWeakPtr(), extension_id));
  did_schedule_reload_ = true;
}

void NeutronExtensionLoader::LoadExtensionForReload(
    const ExtensionId& extension_id,
    const base::FilePath& path) {
  DoLoadExtensionForReload(extension_id, path);
}

void NeutronExtensionLoader::LoadExtensionForReloadWithQuietFailure(
    const ExtensionId& extension_id,
    const base::FilePath& path) {
  DoLoadExtensionForReload(extension_id, path);
}

void NeutronExtensionLoader::ShowExtensionDisabledError(
    const Extension* extension,
    bool is_remote_install) {}

bool NeutronExtensionLoader::CanEnableExtension(const Extension* extension) {
  return true;
}

bool NeutronExtensionLoader::CanDisableExtension(const Extension* extension) {
  // Extensions cannot be disabled by the user.
  return false;
}

void NeutronExtensionLoader::GrantActivePermissions(
    const Extension* extension) {
  NOTIMPLEMENTED();
}
