...
NeutronNavigationThrottle::NeutronNavigationThrottle(
    content::NavigationThrottleRegistry& registry)
    : content::NavigationThrottle(registry) {}

NeutronNavigationThrottle::~NeutronNavigationThrottle() = default;

const char* NeutronNavigationThrottle::GetNameForLogging() {
  return "NeutronNavigationThrottle";
}

content::NavigationThrottle::ThrottleCheckResult
NeutronNavigationThrottle::WillStartRequest() {
...
content::NavigationThrottle::ThrottleCheckResult
NeutronNavigationThrottle::WillRedirectRequest() {
  auto* handle = navigation_handle();
  auto* contents = handle->GetWebContents();
  if (!contents) {
    NOTREACHED();
