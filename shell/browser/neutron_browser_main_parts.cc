...
#include "shell/browser/api/neutron_api_utility_process.h"
#include "shell/browser/browser.h"
#include "shell/browser/browser_process_impl.h"
#include "shell/browser/neutron_browser_client.h"
#include "shell/browser/neutron_browser_context.h"
#include "shell/browser/neutron_web_ui_controller_factory.h"
...
#else
#include "shell/browser/ui/views/neutron_views_delegate.h"
#endif
...
// static
NeutronBrowserMainParts* NeutronBrowserMainParts::self_ = nullptr;

NeutronBrowserMainParts::NeutronBrowserMainParts()
  ...
  DCHECK(!self_) << "Cannot have two NeutronBrowserMainParts";
  self_ = this;
}

NeutronBrowserMainParts::~NeutronBrowserMainParts() = default;

// static
NeutronBrowserMainParts* NeutronBrowserMainParts::Get() {
  DCHECK(self_);
  return self_;
}

bool NeutronBrowserMainParts::SetExitCode(int code) {
  if (!exit_code_)
    return false;
...
int NeutronBrowserMainParts::GetExitCode() const {
  return exit_code_.value_or(content::RESULT_CODE_NORMAL_EXIT);
}

int NeutronBrowserMainParts::PreEarlyInitialization() {
...
void NeutronBrowserMainParts::PostEarlyInitialization() {
  ...
  neutron_bindings_->BindTo(isolate, node_env_->process_object());
...
int NeutronBrowserMainParts::PreCreateThreads() {
  if (!views::LayoutProvider::Get()) {
    layout_provider_ = std::make_unique<views::LayoutProvider>();
  }

  // Fetch the system locale for Neutron.
#if BUILDFLAG(IS_MAC)
  ...
  // Initialize the app locale for Neutron and Chromium.
  std::string app_locale = l10n_util::GetApplicationLocale(loaded_locale);
  NeutronBrowserClient::SetApplicationLocale(app_locale);
...
void NeutronBrowserMainParts::PostCreateThreads() {
...
void NeutronBrowserMainParts::PostDestroyThreads() {
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
...
void NeutronBrowserMainParts::ToolkitInitialized() {
#if BUILDFLAG(IS_LINUX)
  auto* linux_ui = ui::GetDefaultLinuxUi();
  CHECK(linux_ui);
  linux_ui_getter_ = std::make_unique<LinuxUiGetterImpl>();

  neutron::InitializeNeutron_gdk_pixbuf(gtk::GetLibGdkPixbuf());
  CHECK(neutron::IsNeutron_gdk_pixbufInitialized())
...
int NeutronBrowserMainParts::PreMainMessageLoopRun() {
  ...
  extensions_browser_client_ =
      std::make_unique<NeutronExtensionsBrowserClient>();
  extensions_browser_client_->Init();
  extensions::ExtensionsBrowserClient::Set(extensions_browser_client_.get());

  extensions::EnsureBrowserContextKeyedServiceFactoriesBuilt();
  extensions::neutron::EnsureBrowserContextKeyedServiceFactoriesBuilt();
#endif
  ...
  content::WebUIControllerFactory::RegisterFactory(
      NeutronWebUIControllerFactory::GetInstance());
...
#if !BUILDFLAG(IS_MAC)
  // The corresponding call in macOS is in NeutronApplicationDelegate.
...
void NeutronBrowserMainParts::WillRunMainMessageLoop(
...
void NeutronBrowserMainParts::PostCreateMainMessageLoop() {
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_MAC)
  std::string app_name = neutron::Browser::Get()->GetName();
#endif
#if BUILDFLAG(IS_LINUX)
  auto shutdown_cb =
...
void NeutronBrowserMainParts::PostMainMessageLoopRun() {
#if BUILDFLAG(IS_MAC)
  FreeAppDelegate();
#endif
  ...
  // Shutdown utility process created with Neutron API before
  ...
  NeutronBrowserContext::DestroyAllContexts();
...
#if !BUILDFLAG(IS_MAC)
void NeutronBrowserMainParts::PreCreateMainMessageLoop() {
  PreCreateMainMessageLoopCommon();
}
#endif

void NeutronBrowserMainParts::PreCreateMainMessageLoopCommon() {
#if BUILDFLAG(IS_MAC)
  InitializeMainNib();
  RegisterURLHandler();
#endif
...
NeutronBrowserMainParts::GetGeolocationControl() {
...
IconManager* NeutronBrowserMainParts::GetIconManager() {
