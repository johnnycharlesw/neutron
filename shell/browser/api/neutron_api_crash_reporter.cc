...
namespace neutron::api::crash_reporter {
...
bool GetClientIdPath(base::FilePath* path) {
  if (base::PathService::Get(neutron::DIR_CRASH_DUMPS, path)) {
...
void Start(const std::string& submit_url,
  ...
  TRACE_EVENT0("neutron", "crash_reporter::Start");
#if !IS_MAS_BUILD()
  if (g_crash_reporter_initialized)
    return;
  g_crash_reporter_initialized = true;
  NeutronCrashReporterClient::Create();
  NeutronCrashReporterClient::Get()->SetUploadUrl(submit_url);
  NeutronCrashReporterClient::Get()->SetCollectStatsConsent(upload_to_server);
  NeutronCrashReporterClient::Get()->SetShouldRateLimit(rate_limit);
  NeutronCrashReporterClient::Get()->SetShouldCompressUploads(compress);
  NeutronCrashReporterClient::Get()->SetGlobalAnnotations(global_extra);
  std::string process_type = is_node_process ? "node" : GetProcessType();
#if BUILDFLAG(IS_LINUX)
  for (const auto& pair : extra)
    neutron::crash_keys::SetCrashKey(pair.first, pair.second);
  {
    neutron::ScopedAllowBlockingForNeutron allow_blocking;
...
scoped_refptr<UploadList> CreateCrashUploadList() {
  ...
  base::PathService::Get(neutron::DIR_CRASH_DUMPS, &crash_dir_path);
...
#if !IS_MAS_BUILD()
  NeutronCrashReporterClient::Get()->SetCollectStatsConsent(upload);
#endif
}
...
bool GetUploadToServer() {
  ...
  return false;
#else
  return NeutronCrashReporterClient::Get()->GetCollectStatsConsent();
#endif
}
...
void Initialize(v8::Local<v8::Object> exports,
  ...
  dict.SetMethod("start", &neutron::api::crash_reporter::Start);
#if IS_MAS_BUILD()
  dict.SetMethod("addExtraParameter", &neutron::api::crash_reporter::NoOp);
  dict.SetMethod("removeExtraParameter", &neutron::api::crash_reporter::NoOp);
#else
  dict.SetMethod("addExtraParameter", &neutron::crash_keys::SetCrashKey);
  dict.SetMethod("removeExtraParameter", &neutron::crash_keys::ClearCrashKey);
#endif
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_crash_reporter, Initialize)
