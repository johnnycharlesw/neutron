...
#include "shell/browser/api/neutron_api_global_shortcut.h"
...
#include "neutron/shell/browser/neutron_browser_context.h"
#include "neutron/shell/common/neutron_constants.h"
...
bool RegisteringMediaKeyForUntrustedClient(const ui::Accelerator& accelerator) {
  return accelerator.IsMediaKey() &&
         !neutron::api::SystemPreferences::IsTrustedAccessibilityClient(false);
}
...
bool GlobalShortcut::Register(const ui::Accelerator& accelerator,
  ...
  if (instance->IsRegistrationHandledExternally()) {
    auto* context = NeutronBrowserContext::GetDefaultBrowserContext();
    ...
    std::string profile_id = prefs->GetString(kNeutronGlobalShortcutsUuid);
    if (profile_id.empty()) {
      profile_id = base::Uuid::GenerateRandomV4().AsLowercaseString();
      prefs->SetString(kNeutronGlobalShortcutsUuid, profile_id);
    }
    ...
    extensions::Command command(
        command_str, base::UTF8ToUTF16("Neutron shortcut " + command_str),
...
void Initialize(v8::Local<v8::Object> exports,
  ...
  dict.Set("globalShortcut", neutron::api::GlobalShortcut::Create(isolate));
}

}  // namespace

NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_global_shortcut, Initialize)
