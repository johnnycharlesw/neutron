...
#include "shell/browser/api/neutron_api_screen.h"
...
static gfx::Rect ScreenToDIPRect(neutron::NativeWindow* window,
                                 const gfx::Rect& rect) {
  HWND hwnd = window ? window->GetAcceleratedWidget() : nullptr;
  return display::win::GetScreenWin()->ScreenToDIPRect(hwnd, rect);
}

static gfx::Rect DIPToScreenRect(neutron::NativeWindow* window,
                                 const gfx::Rect& rect) {
  HWND hwnd = window ? window->GetAcceleratedWidget() : nullptr;
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_screen, Initialize)
