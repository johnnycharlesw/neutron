...
class Notification final : public gin_helper::DeprecatedWrappable<Notification>,
 ...
 protected:
  ...
  const std::vector<neutron::NotificationAction>& actions() const {
    return actions_;
  }
  ...
  void SetActions(const std::vector<neutron::NotificationAction>& actions);
 ...
 private:
  ...
  std::vector<neutron::NotificationAction> actions_;
  std::u16string close_button_text_;
  std::u16string toast_xml_;

  raw_ptr<neutron::NotificationPresenter> presenter_;

  base::WeakPtr<neutron::Notification> notification_;
};
