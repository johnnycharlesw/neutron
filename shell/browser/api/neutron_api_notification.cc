...
#include "shell/browser/api/neutron_api_notification.h"
...
template <>
struct Converter<neutron::NotificationAction> {
  static bool FromV8(v8::Isolate* isolate,
                     v8::Local<v8::Value> val,
                     neutron::NotificationAction* out) {
  ...
  static v8::Local<v8::Value> ToV8(v8::Isolate* isolate,
                                   neutron::NotificationAction val) {
...
Notification::Notification(gin::Arguments* args) {
  presenter_ = static_cast<NeutronBrowserClient*>(NeutronBrowserClient::Get())
...
void Notification::SetActions(
    const std::vector<neutron::NotificationAction>& actions) {
  actions_ = actions;
}
...
bool Notification::IsSupported() {
  return !!static_cast<NeutronBrowserClient*>(NeutronBrowserClient::Get())
               ->GetNotificationPresenter();
}
...
using neutron::api::Notification;
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_notification, Initialize)
