...
using SharingItem = neutron::NeutronMenuModel::SharingItem;
...
Menu::Menu(gin::Arguments* args)
    : model_(std::make_unique<NeutronMenuModel>(this)) {
  model_->AddObserver(this);

#if BUILDFLAG(IS_MAC)
  gin_helper::Dictionary options;
  if (args->GetNext(&options)) {
    NeutronMenuModel::SharingItem item;
...
bool Menu::GetSharingItemForCommandId(
    int command_id,
    NeutronMenuModel::SharingItem* item) const {
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_menu, Initialize)
