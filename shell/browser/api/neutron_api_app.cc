...
#include "shell/browser/api/neutron_api_menu.h"
...
#include "shell/browser/ui/cocoa/neutron_bundle_mover.h"
...
using neutron::Browser;

namespace gin {

#if BUILDFLAG(IS_WIN)
template <>
struct Converter<neutron::ProcessIntegrityLevel> {
  static v8::Local<v8::Value> ToV8(v8::Isolate* isolate,
                                   neutron::ProcessIntegrityLevel value) {
    switch (value) {
      case neutron::ProcessIntegrityLevel::kUntrusted:
        return StringToV8(isolate, "untrusted");
      case neutron::ProcessIntegrityLevel::kLow:
        return StringToV8(isolate, "low");
      case neutron::ProcessIntegrityLevel::kMedium:
        return StringToV8(isolate, "medium");
      case neutron::ProcessIntegrityLevel::kHigh:
...
bool NotificationCallbackWrapper(
  ...
  // neutron app instance. This also removes the switch after use to prevent
...
App::App() {
  static_cast<NeutronBrowserClient*>(NeutronBrowserClient::Get())
      ->set_delegate(this);
  Browser::Get()->AddObserver(this);

  auto unsafe_pid = content::ChildProcessId::FromUnsafeValue(
      content::ChildProcessHost::kInvalidUniqueID);
  auto process_metric = std::make_unique<neutron::ProcessMetric>(
...
void App::OnQuit() {
  const int exitCode = NeutronBrowserMainParts::Get()->GetExitCode();
  Emit("quit", exitCode);
...
void App::AllowCertificateError(
  ...
  auto adapted_callback =
      neutron::AdaptCallbackForRepeating(std::move(callback));
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
  v8::HandleScope handle_scope(isolate);
...
#endif
  app_metrics_[pid] = std::make_unique<neutron::ProcessMetric>(
...
#if BUILDFLAG(IS_WIN)
  return base_name != FILE_PATH_LITERAL("neutron.exe");
...
bool App::RequestSingleInstanceLock(gin::Arguments* args) {
...
#if BUILDFLAG(IS_WIN)
  const std::string program_name = neutron::Browser::Get()->GetName();
  ...
  // already running neutron app instance if it exists.
  // The activation token received from the launching app is used later when
  // activating the existing neutron app instance window.
...
bool App::Relaunch(gin::Arguments* js_args) {
  ...
  if (!override_argv) {
    const relauncher::StringVector& argv =
        neutron::NeutronCommandLine::argv();
...
void App::SetUserAgentFallback(const std::string& user_agent) {
  NeutronBrowserClient::Get()->SetUserAgent(user_agent);
}
...
std::string App::GetUserAgentFallback() {
  return NeutronBrowserClient::Get()->GetUserAgent();
}

#if BUILDFLAG(IS_MAC)
bool App::MoveToApplicationsFolder(gin_helper::ErrorThrower thrower,
                                   gin::Arguments* args) {
  return NeutronBundleMover::Move(thrower, args);
}

bool App::IsInApplicationsFolder() {
  return NeutronBundleMover::IsCurrentAppInApplicationsFolder();
}
...
void DockSetMenu(neutron::api::Menu* menu) {
...
void Initialize(v8::Local<v8::Object> exports,
  ...
  dict.Set("app", neutron::api::App::Create(isolate));
}

}  // namespace

NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_app, Initialize)
