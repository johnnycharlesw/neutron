...
#include "shell/browser/api/neutron_api_debugger.h"
...
template <>
struct Converter<neutron::api::WebContents::Type> {
  static v8::Local<v8::Value> ToV8(v8::Isolate* isolate,
                                   neutron::api::WebContents::Type val) {
    using Type = neutron::api::WebContents::Type;
  ...
  static bool FromV8(v8::Isolate* isolate,
                     v8::Local<v8::Value> val,
                     neutron::api::WebContents::Type* out) {
    using Val = neutron::api::WebContents::Type;
    static constexpr auto Lookup =
        base::MakeFixedFlatMap<std::string_view, Val>({
...
const void* kNeutronApiWebContentsKey = &kNeutronApiWebContentsKey;
...
PrefService* GetPrefService(content::WebContents* web_contents) {
  ...
  return static_cast<neutron::NeutronBrowserContext*>(context)->prefs();
}
...
#endif
{
  ...
  // See: https://github.com/neutron/neutron/issues/49256
  ...
  web_contents->SetUserData(kNeutronApiWebContentsKey,
...
#endif
{
  ...
  if (is_guest()) {
    ...
    // to be transparent, that will be set at neutron_api_browser_window.cc#L57
...
void WebContents::InitWithSessionAndOptions(
  ...
  for (auto const& language : neutron::GetPreferredLanguages()) {
...
#if BUILDFLAG(ENABLE_PRINTING)
  PrintViewManagerNeutron::CreateForWebContents(web_contents.get());
...
// neutron_api_web_contents_mac.mm, as it requires calling into objective-C
// code.
...
content::JavaScriptDialogManager* WebContents::GetJavaScriptDialogManager(
  ...
  // content::WebContents from calling methods on the Neutron WebContents in
  // the event that the Neutron one is destroyed before the content one, as
...
void WebContents::DidChangeThemeColor() {
  ...
  if (theme_color) {
    Emit("did-change-theme-color", neutron::ToRGBHex(theme_color.value()));
  } else {
...
void OnGetDeviceNameToUse(base::WeakPtr<content::WebContents> web_contents,
  ...
  // PrintViewManagerNeutron class does not do null check.
  if (!web_contents) {
    if (print_callback)
  ...
  auto* print_view_manager = PrintViewManagerNeutron::FromWebContents(
...
void OnPDFCreated(gin_helper::Promise<v8::Local<v8::Value>> promise,
  ...
  v8::Local<v8::Value> buffer =
      neutron::Buffer::Copy(isolate, *data).ToLocalChecked();
...
v8::Local<v8::Promise> WebContents::PrintToPDF(const base::Value& settings) {
  ...
  auto* manager = PrintViewManagerNeutron::FromWebContents(web_contents());
  if (!manager) {
...
v8::Local<v8::Value> WebContents::Debugger(v8::Isolate* isolate) {
  if (!debugger_) {
    debugger_ = neutron::api::Debugger::Create(isolate, web_contents());
  }
...
v8::Local<v8::Promise> WebContents::GetProcessMemoryInfo(v8::Isolate* isolate) {
  ...
  memory_instrumentation::MemoryInstrumentation::GetInstance()
      ->RequestGlobalDumpForPid(
          ...
          base::BindOnce(&NeutronBindings::DidReceiveMemoryDump,
...
v8::Local<v8::Promise> WebContents::TakeHeapSnapshot(
  ...
  // this function.
  auto neutron_renderer =
      std::make_unique<mojo::Remote<mojom::NeutronRenderer>>();
  frame_host->GetRemoteInterfaces()->GetInterface(
      neutron_renderer->BindNewPipeAndPassReceiver());
  auto* raw_ptr = neutron_renderer.get();
  (*raw_ptr)->TakeHeapSnapshot(
      mojo::WrapPlatformFile(base::ScopedPlatformFile(file.TakePlatformFile())),
      base::BindOnce(
          [](mojo::Remote<mojom::NeutronRenderer>* ep,
          ...
          base::Owned(std::move(neutron_renderer)), std::move(promise)));
  return handle;
}
...
NeutronBrowserContext* WebContents::GetBrowserContext() const {
  return static_cast<NeutronBrowserContext*>(
...
WebContents* WebContents::From(content::WebContents* web_contents) {
  ...
  auto* data = static_cast<UserDataLink*>(
      web_contents->GetUserData(kNeutronApiWebContentsKey));
...
using neutron::api::GetAllWebContents;
using neutron::api::WebContents;
using neutron::api::WebFrameMain;
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_web_contents, Initialize)
