...
void RegisterSchemesAsPrivileged(gin_helper::ErrorThrower thrower,
  ...
  AppendSchemesToCmdLine(neutron::switches::kSecureSchemes, secure_schemes);
  AppendSchemesToCmdLine(neutron::switches::kBypassCSPSchemes,
                         cspbypassing_schemes);
  AppendSchemesToCmdLine(neutron::switches::kCORSSchemes, cors_schemes);
  AppendSchemesToCmdLine(neutron::switches::kFetchSchemes, fetch_schemes);
  AppendSchemesToCmdLine(neutron::switches::kServiceWorkerSchemes,
                         service_worker_schemes);
  AppendSchemesToCmdLine(neutron::switches::kStandardSchemes,
                         GetStandardSchemes());
  AppendSchemesToCmdLine(neutron::switches::kStreamingSchemes,
                         GetStreamingSchemes());
  AppendSchemesToCmdLine(neutron::switches::kCodeCacheSchemes,
                         GetCodeCacheSchemes());
}
...
void RegisterSchemesAsPrivileged(gin_helper::ErrorThrower thrower,
  ...
  neutron::api::RegisterSchemesAsPrivileged(thrower, val);
}
...
void Initialize(v8::Local<v8::Object> exports,
  ...
  dict.Set("Protocol",
           neutron::api::Protocol::GetConstructor(isolate, context));
  dict.SetMethod("registerSchemesAsPrivileged", &RegisterSchemesAsPrivileged);
  dict.SetMethod("getStandardSchemes", &neutron::api::GetStandardSchemes);
}

}  // namespace

NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_protocol, Initialize)
