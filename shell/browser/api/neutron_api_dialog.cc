...
int ShowMessageBoxSync(const neutron::MessageBoxSettings& settings) {
  return neutron::ShowMessageBoxSync(settings);
}
...
v8::Local<v8::Promise> ShowMessageBox(
    ...
    const neutron::MessageBoxSettings& settings) {
  gin_helper::Promise<gin_helper::Dictionary> promise(isolate);
  v8::Local<v8::Promise> handle = promise.GetHandle();

  neutron::ShowMessageBox(
...
void Initialize(v8::Local<v8::Object> exports,
  ...
  dict.SetMethod("_closeMessageBox", &neutron::CloseMessageBox);
  dict.SetMethod("showErrorBox", &neutron::ShowErrorBox);
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_dialog, Initialize)
