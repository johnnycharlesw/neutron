...
#include "shell/browser/api/neutron_api_cookies.h"
#include "shell/browser/api/neutron_api_data_pipe_holder.h"
#include "shell/browser/api/neutron_api_download_item.h"
#include "shell/browser/api/neutron_api_net_log.h"
#include "shell/browser/api/neutron_api_protocol.h"
#include "shell/browser/api/neutron_api_service_worker_context.h"
...
#include "shell/browser/api/neutron_api_extensions.h"
#endif
...
const void* kNeutronApiSessionKey = &kNeutronApiSessionKey;
...
Session::Session(v8::Isolate* isolate, NeutronBrowserContext* browser_context)
    : isolate_(isolate),
      network_emulation_token_(base::UnguessableToken::Create()),
      browser_context_{
          raw_ref<NeutronBrowserContext>::from_ptr(browser_context)} {
...
void Session::SetPermissionRequestHandler(v8::Local<v8::Value> val,
  ...
  if (val->IsNull()) {
    permission_manager->SetPermissionRequestHandler(
        NeutronPermissionManager::RequestHandler());
    return;
  }
  auto handler = std::make_unique<NeutronPermissionManager::RequestHandler>();
  if (!gin::ConvertFromV8(args->isolate(), val, handler.get())) {
    args->ThrowTypeError("Must pass null or function");
    return;
  }
  permission_manager->SetPermissionRequestHandler(base::BindRepeating(
      [](NeutronPermissionManager::RequestHandler* handler,
         content::WebContents* web_contents,
...
void Session::SetPermissionCheckHandler(v8::Local<v8::Value> val,
                                        gin::Arguments* args) {
  NeutronPermissionManager::CheckHandler handler;
...
void Session::SetDevicePermissionHandler(v8::Local<v8::Value> val,
                                         gin::Arguments* args) {
  NeutronPermissionManager::DeviceCheckHandler handler;
  if (!(val->IsNull() || gin::ConvertFromV8(args->isolate(), val, &handler))) {
    args->ThrowTypeError("Must pass null or function");
    return;
  }
...
void Session::SetUSBProtectedClassesHandler(v8::Local<v8::Value> val,
                                            gin::Arguments* args) {
  NeutronPermissionManager::ProtectedUSBHandler handler;
  if (!(val->IsNull() || gin::ConvertFromV8(args->isolate(), val, &handler))) {
    args->ThrowTypeError("Must pass null or function");
    return;
  }
...
void Session::SetBluetoothPairingHandler(v8::Local<v8::Value> val,
                                         gin::Arguments* args) {
  NeutronPermissionManager::BluetoothPairingHandler handler;
  if (!(val->IsNull() || gin::ConvertFromV8(args->isolate(), val, &handler))) {
    args->ThrowTypeError("Must pass null or function");
    return;
  }
...
static void StartPreconnectOnUI(NeutronBrowserContext* browser_context,
                                const GURL& url,
...
gin::WeakCell<Session>* Session::FromBrowserContext(
  ...
  auto* data =
      static_cast<UserDataLink*>(context->GetUserData(kNeutronApiSessionKey));
...
Session* Session::FromOrCreate(v8::Isolate* isolate,
                               content::BrowserContext* context) {
  if (NeutronBrowserContext::IsValidContext(context))
    return FromOrCreate(isolate, static_cast<NeutronBrowserContext*>(context));
  DCHECK(false);
  return {};
}
...
Session* Session::FromPartition(v8::Isolate* isolate,
  ...
  if (partition.empty()) {
    browser_context =
        NeutronBrowserContext::GetDefaultBrowserContext(std::move(options));
  } else if (partition.starts_with(kPersistPrefix)) {
    std::string name = partition.substr(8);
    browser_context =
        NeutronBrowserContext::From(name, false, std::move(options));
  } else {
    browser_context =
        NeutronBrowserContext::From(partition, true, std::move(options));
  }
...
Session* Session::FromPath(gin::Arguments* args,
  ...
  browser_context =
      NeutronBrowserContext::FromPath(std::move(path), std::move(options));
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_browser_session, Initialize)
