...
PrintViewManagerNeutron::PrintViewManagerNeutron(
    content::WebContents* web_contents)
    : printing::PrintViewManagerBase(web_contents),
      content::WebContentsUserData<PrintViewManagerNeutron>(*web_contents) {}

PrintViewManagerNeutron::~PrintViewManagerNeutron() = default;

// static
void PrintViewManagerNeutron::BindPrintManagerHost(
  ...
  auto* print_manager = PrintViewManagerNeutron::FromWebContents(web_contents);
  if (!print_manager)
    return;
...
void PrintViewManagerNeutron::DidPrintToPdf(
    int cookie,
...
void PrintViewManagerNeutron::PrintToPdf(
    content::RenderFrameHost* rfh,
  ...
  print_to_pdf::PdfPrintJob::StartJob(
      ...
      base::BindOnce(&PrintViewManagerNeutron::DidPrintToPdf,
                     weak_factory_.GetWeakPtr(), cookie, std::move(callback)));
}

void PrintViewManagerNeutron::GetDefaultPrintSettings(
...
void PrintViewManagerNeutron::ScriptedPrint(
...
void PrintViewManagerNeutron::DidGetPrintedPagesCount(int32_t cookie,
...
WEB_CONTENTS_USER_DATA_KEY_IMPL(PrintViewManagerNeutron);
