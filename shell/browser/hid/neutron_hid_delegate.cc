...
neutron::HidChooserContext* GetChooserContext(
    content::BrowserContext* browser_context) {
  if (!browser_context)
    return nullptr;
  return neutron::HidChooserContextFactory::GetForBrowserContext(
      browser_context);
}
...
class NeutronHidDelegate::ContextObservation
    : private HidChooserContext::DeviceObserver {
 public:
  ContextObservation(NeutronHidDelegate* parent,
                     content::BrowserContext* browser_context)
 ...
 private:
  ...
  const raw_ptr<NeutronHidDelegate> parent_;
...
NeutronHidDelegate::NeutronHidDelegate() = default;

NeutronHidDelegate::~NeutronHidDelegate() = default;

std::unique_ptr<content::HidChooser> NeutronHidDelegate::RunChooser(
...
bool NeutronHidDelegate::CanRequestDevicePermission(
    content::BrowserContext* browser_context,
...
bool NeutronHidDelegate::HasDevicePermission(
    content::BrowserContext* browser_context,
    content::RenderFrameHost* render_frame_host,
    const url::Origin& origin,
...
void NeutronHidDelegate::RevokeDevicePermission(
    content::BrowserContext* browser_context,
    content::RenderFrameHost* render_frame_host,
    const url::Origin& origin,
    const device::mojom::HidDeviceInfo& device) {
...
device::mojom::HidManager* NeutronHidDelegate::GetHidManager(
    content::BrowserContext* browser_context) {
  if (!browser_context)
    return nullptr;
...
void NeutronHidDelegate::AddObserver(content::BrowserContext* browser_context,
                                      Observer* observer) {
  if (!browser_context)
    return;
...
void NeutronHidDelegate::RemoveObserver(
    content::BrowserContext* browser_context,
...
const device::mojom::HidDeviceInfo* NeutronHidDelegate::GetDeviceInfo(
    content::BrowserContext* browser_context,
...
bool NeutronHidDelegate::IsFidoAllowedForOrigin(
    content::BrowserContext* browser_context,
    const url::Origin& origin) {
  auto* chooser_context = GetChooserContext(browser_context);
...
bool NeutronHidDelegate::IsServiceWorkerAllowedForOrigin(
    const url::Origin& origin) {
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
...
NeutronHidDelegate::ContextObservation*
NeutronHidDelegate::GetContextObserver(
    content::BrowserContext* browser_context) {
...
HidChooserController* NeutronHidDelegate::ControllerForFrame(
    content::RenderFrameHost* render_frame_host) {
  auto mapping = controller_map_.find(render_frame_host);
  return mapping == controller_map_.end() ? nullptr : mapping->second.get();
}

HidChooserController* NeutronHidDelegate::AddControllerForFrame(
    content::RenderFrameHost* render_frame_host,
    std::vector<blink::mojom::HidDeviceFilterPtr> filters,
    std::vector<blink::mojom::HidDeviceFilterPtr> exclusion_filters,
    content::HidChooser::Callback callback) {
...
void NeutronHidDelegate::DeleteControllerForFrame(
    content::RenderFrameHost* render_frame_host) {
