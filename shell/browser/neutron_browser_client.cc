...
#include "shell/browser/neutron_autofill_driver_factory.h"
#include "shell/browser/neutron_browser_context.h"
#include "shell/browser/neutron_browser_main_parts.h"
#include "shell/browser/neutron_navigation_throttle.h"
#include "shell/browser/neutron_plugin_info_host_impl.h"
#include "shell/browser/neutron_speech_recognition_manager_delegate.h"
#include "shell/browser/neutron_web_contents_utility_handler_impl.h"
...
#include "shell/browser/neutron_crypto_module_delegate_nss.h"
...
#include "shell/browser/neutron_pdf_document_helper_client.h"
...
NeutronBrowserClient* g_browser_client = nullptr;
...
// static
NeutronBrowserClient* NeutronBrowserClient::Get() {
  return g_browser_client;
}

// static
void NeutronBrowserClient::SetApplicationLocale(const std::string& locale) {
...
NeutronBrowserClient::NeutronBrowserClient() {
...
content::WebContents* NeutronBrowserClient::GetWebContentsFromProcessID(
...
content::SiteInstance* NeutronBrowserClient::GetSiteInstanceFromAffinity(
...
bool NeutronBrowserClient::IsRendererSubFrame(
    content::ChildProcessId process_id) const {
  return renderer_is_subframe_.contains(process_id);
}

void NeutronBrowserClient::RenderProcessWillLaunch(
...
NeutronBrowserClient::CreateSpeechRecognitionManagerDelegate() {
  return new NeutronSpeechRecognitionManagerDelegate;
}

content::TtsPlatform* NeutronBrowserClient::GetTtsPlatform() {
  return nullptr;
}

void NeutronBrowserClient::OverrideWebPreferences(
  ...
  prefs->allow_universal_access_from_file_urls =
      neutron::fuses::IsGrantFileProtocolExtraPrivilegesEnabled();
...
bool NeutronBrowserClient::WebPreferencesNeedUpdateForColorRelatedStateChanges(
    content::WebContents& web_contents,
...
void NeutronBrowserClient::RegisterPendingSiteInstance(
...
void NeutronBrowserClient::AppendExtraCommandLineSwitches(
  ...
  if (process_type == ::switches::kRendererProcess) {
    ...
    if (env->HasVar("ELECTRON_PROFILE_INIT_SCRIPTS")) {
      command_line->AppendSwitch("profile-neutron-init");
    }
...
std::string NeutronBrowserClient::GetGeolocationApiKey() {
  auto env = base::Environment::Create();
...
NeutronBrowserClient::GetGeneratedCodeCacheSettings(
    content::BrowserContext* context) {
...
void NeutronBrowserClient::AllowCertificateError(
    content::WebContents* web_contents,
    int cert_error,
...
base::OnceClosure NeutronBrowserClient::SelectClientCertificate(
    content::BrowserContext* browser_context,
    int process_id,
    content::WebContents* web_contents,
...
bool NeutronBrowserClient::CanCreateWindow(
...
NeutronBrowserClient::CreateWindowForVideoPictureInPicture(
...
void NeutronBrowserClient::GetAdditionalAllowedSchemesForFileSystem(
...
void NeutronBrowserClient::GetAdditionalWebUISchemes(
    std::vector<std::string>* additional_schemes) {
  additional_schemes->push_back(content::kChromeDevToolsScheme);
}

void NeutronBrowserClient::SiteInstanceGotProcessAndSite(
    content::SiteInstance* site_instance) {
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
  auto* browser_context =
      static_cast<NeutronBrowserContext*>(site_instance->GetBrowserContext());
...
bool NeutronBrowserClient::IsSuitableHost(
    content::RenderProcessHost* process_host,
    const GURL& site_url) {
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
...
bool NeutronBrowserClient::ShouldUseProcessPerSite(
    content::BrowserContext* browser_context,
    const GURL& effective_url) {
#if BUILDFLAG(ENABLE_ELECTRON_EXTENSIONS)
  const extensions::Extension* extension =
...
void NeutronBrowserClient::GetMediaDeviceIDSalt(
    content::RenderFrameHost* rfh,
...
NeutronBrowserClient::OverrideSystemLocationProvider() {
#if BUILDFLAG(OVERRIDE_LOCATION_PROVIDER)
...
void NeutronBrowserClient::ConfigureNetworkContextParams(
    content::BrowserContext* browser_context,
    bool in_memory,
...
NeutronBrowserClient::GetSystemNetworkContext() {
  DCHECK_CURRENTLY_ON(BrowserThread::UI);
...
NeutronBrowserClient::CreateBrowserMainParts(bool /* is_integration_test */) {
  return std::make_unique<NeutronBrowserMainParts>();
}

void NeutronBrowserClient::WebNotificationAllowed(
    content::RenderFrameHost* rfh,
...
void NeutronBrowserClient::RenderProcessHostDestroyed(
    content::RenderProcessHost* host) {
...
void NeutronBrowserClient::RenderProcessReady(
    content::RenderProcessHost* host) {
  if (delegate_) {
...
void NeutronBrowserClient::RenderProcessExited(
    content::RenderProcessHost* host,
...
bool NeutronBrowserClient::HandleExternalProtocol(
    const GURL& url,
...
void NeutronBrowserClient::CreateThrottlesForNavigation(
    content::NavigationThrottleRegistry& registry) {
  registry.AddThrottle(std::make_unique<NeutronNavigationThrottle>(registry));
...
content::MediaObserver* NeutronBrowserClient::GetMediaObserver() {
  return MediaCaptureDevicesDispatcher::GetInstance();
}

std::unique_ptr<content::DevToolsManagerDelegate>
NeutronBrowserClient::CreateDevToolsManagerDelegate() {
  return std::make_unique<DevToolsManagerDelegate>();
}

NotificationPresenter* NeutronBrowserClient::GetNotificationPresenter() {
...
NeutronBrowserClient::GetPlatformNotificationService() {
...
base::FilePath NeutronBrowserClient::GetDefaultDownloadDirectory() {
...
NeutronBrowserClient::GetSystemSharedURLLoaderFactory() {
  if (!g_browser_process)
    return nullptr;
  return g_browser_process->shared_url_loader_factory();
}

void NeutronBrowserClient::OnNetworkServiceCreated(
...
NeutronBrowserClient::GetNetworkContextsParentDirectory() {
...
std::string NeutronBrowserClient::GetProduct() {
  return "Chrome/" CHROME_VERSION_STRING;
}

std::string NeutronBrowserClient::GetUserAgent() {
  if (user_agent_override_.empty())
    return GetApplicationUserAgent();
  return user_agent_override_;
}

void NeutronBrowserClient::SetUserAgent(const std::string& user_agent) {
  user_agent_override_ = user_agent;
}

blink::UserAgentMetadata NeutronBrowserClient::GetUserAgentMetadata() {
  return embedder_support::GetUserAgentMetadata();
}

mojo::PendingRemote<network::mojom::URLLoaderFactory>
NeutronBrowserClient::CreateNonNetworkNavigationURLLoaderFactory(
...
void NeutronBrowserClient::RegisterNonNetworkSubresourceURLLoaderFactories(
  ...
  extensions::NeutronExtensionWebContentsObserver* web_observer =
      extensions::NeutronExtensionWebContentsObserver::FromWebContents(
          web_contents);

  // There is nothing to do if no NeutronExtensionWebContentsObserver is
...
bool NeutronBrowserClient::ShouldTreatURLSchemeAsFirstPartyWhenTopLevel(
...
bool NeutronBrowserClient::WillInterceptWebSocket(
...
void NeutronBrowserClient::CreateWebSocket(
    content::RenderFrameHost* frame,
...
void NeutronBrowserClient::WillCreateURLLoaderFactory(
    content::BrowserContext* browser_context,
    content::RenderFrameHost* frame_host,
    int render_process_id,
  ...
  //
  // Note that in Neutron we allow webRequest to capture requests sent from
...
NeutronBrowserClient::WillCreateURLLoaderRequestInterceptors(
...
void NeutronBrowserClient::OverrideURLLoaderFactoryParams(
    content::BrowserContext* browser_context,
...
void NeutronBrowserClient::RegisterAssociatedInterfaceBindersForServiceWorker(
    const content::ServiceWorkerVersionBaseInfo& service_worker_version_info,
    blink::AssociatedInterfaceRegistry& associated_registry) {
  CHECK(service_worker_version_info.process_id !=
        content::ChildProcessHost::kInvalidUniqueID);
  associated_registry.AddInterface<mojom::NeutronApiIPC>(
      base::BindRepeating(&NeutronApiSWIPCHandlerImpl::BindReceiver,
...
void NeutronBrowserClient::
      ...
      associated_registry.AddInterface<mojom::NeutronApiIPC>(
          base::BindRepeating(
              [](content::RenderFrameHost* render_frame_host,
                 mojo::PendingAssociatedReceiver<mojom::NeutronApiIPC>
                     receiver) {
                NeutronApiIPCHandlerImpl::Create(render_frame_host,
                                                  std::move(receiver));
              },
              &render_frame_host));
    }
  ...
  associated_registry.AddInterface<mojom::NeutronWebContentsUtility>(
      base::BindRepeating(
          [](content::RenderFrameHost* render_frame_host,
             mojo::PendingAssociatedReceiver<mojom::NeutronWebContentsUtility>
                 receiver) {
            NeutronWebContentsUtilityHandlerImpl::Create(render_frame_host,
                                                          std::move(receiver));
          },
          &render_frame_host));

  associated_registry.AddInterface<mojom::NeutronAutofillDriver>(
      base::BindRepeating(
          [](content::RenderFrameHost* render_frame_host,
             mojo::PendingAssociatedReceiver<mojom::NeutronAutofillDriver>
                 receiver) {
          ...
          &render_frame_host));
#if BUILDFLAG(ENABLE_PLUGINS)
  associated_registry.AddInterface<mojom::NeutronPluginInfoHost>(
      base::BindRepeating(
          [](content::RenderFrameHost* render_frame_host,
             mojo::PendingAssociatedReceiver<mojom::NeutronPluginInfoHost>
                 receiver) {
            mojo::MakeSelfOwnedAssociatedReceiver(
                std::make_unique<NeutronPluginInfoHostImpl>(),
                std::move(receiver));
          },
          &render_frame_host));
#endif
      ...
      base::BindRepeating(
          [](content::RenderFrameHost* render_frame_host,
             mojo::PendingAssociatedReceiver<printing::mojom::PrintManagerHost>
                 receiver) {
            PrintViewManagerNeutron::BindPrintManagerHost(std::move(receiver),
                                                           render_frame_host);
          },
          &render_frame_host));
#endif
  ...
  associated_registry.AddInterface<pdf::mojom::PdfHost>(base::BindRepeating(
      [](content::RenderFrameHost* render_frame_host,
        ...
        pdf::PDFDocumentHelper::BindPdfHost(
            std::move(receiver), render_frame_host,
            std::make_unique<NeutronPDFDocumentHelperClient>());
      },
      &render_frame_host));
#endif
}

std::string NeutronBrowserClient::GetApplicationLocale() {
  return BrowserThread::CurrentlyOn(BrowserThread::IO)
             ? *g_io_thread_application_locale
             : *g_application_locale;
}

bool NeutronBrowserClient::ShouldEnableStrictSiteIsolation() {
  // Enable site isolation. It is off by default in Chromium <= 69.
  return true;
}

void NeutronBrowserClient::BindHostReceiverForRenderer(
...
void NeutronBrowserClient::ExposeInterfacesToRenderer(
...
void NeutronBrowserClient::RegisterBrowserInterfaceBindersForFrame(
    content::RenderFrameHost* render_frame_host,
...
#if BUILDFLAG(IS_LINUX)
void NeutronBrowserClient::GetAdditionalMappedFilesForChildProcess(
...
NeutronBrowserClient::CreateLoginDelegate(
...
NeutronBrowserClient::CreateURLLoaderThrottles(
    const network::ResourceRequest& request,
    content::BrowserContext* browser_context,
...
NeutronBrowserClient::GetPluginMimeTypesWithExternalHandlers(
    content::BrowserContext* browser_context) {
...
content::SerialDelegate* NeutronBrowserClient::GetSerialDelegate() {
  if (!serial_delegate_)
    serial_delegate_ = std::make_unique<NeutronSerialDelegate>();
  return serial_delegate_.get();
}

content::BluetoothDelegate* NeutronBrowserClient::GetBluetoothDelegate() {
  if (!bluetooth_delegate_)
    bluetooth_delegate_ = std::make_unique<NeutronBluetoothDelegate>();
  return bluetooth_delegate_.get();
}

content::UsbDelegate* NeutronBrowserClient::GetUsbDelegate() {
  if (!usb_delegate_)
    usb_delegate_ = std::make_unique<NeutronUsbDelegate>();
...
void NeutronBrowserClient::RegisterBrowserInterfaceBindersForServiceWorker(
    content::BrowserContext* browser_context,
    const content::ServiceWorkerVersionBaseInfo& service_worker_version_info,
...
NeutronBrowserClient::GetGeolocationSystemPermissionManager() {
  return device::GeolocationSystemPermissionManager::GetInstance();
}
#endif

content::HidDelegate* NeutronBrowserClient::GetHidDelegate() {
  if (!hid_delegate_)
    hid_delegate_ = std::make_unique<NeutronHidDelegate>();
  return hid_delegate_.get();
}

content::WebAuthenticationDelegate*
NeutronBrowserClient::GetWebAuthenticationDelegate() {
  if (!web_authentication_delegate_) {
    web_authentication_delegate_ =
        std::make_unique<NeutronWebAuthenticationDelegate>();
  }
