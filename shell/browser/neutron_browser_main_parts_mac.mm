...
static NeutronApplicationDelegate* __strong delegate_;

void NeutronBrowserMainParts::PreCreateMainMessageLoop() {
  // Set our own application delegate.
  delegate_ = [[NeutronApplicationDelegate alloc] init];
  ...
  const bool geolocationDisabled =
      NeutronPermissionManager::IsGeolocationDisabledViaCommandLine();
...
void NeutronBrowserMainParts::FreeAppDelegate() {
  delegate_ = nil;
  [NSApp setDelegate:nil];
}

void NeutronBrowserMainParts::RegisterURLHandler() {
  [[AtomApplication sharedApplication] registerURLHandler];
}

// Replicates NSApplicationMain, but doesn't start a run loop.
void NeutronBrowserMainParts::InitializeMainNib() {
...
std::string NeutronBrowserMainParts::GetCurrentSystemLocale() {
