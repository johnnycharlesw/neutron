...
class IPCBase : public gin_helper::DeprecatedWrappable<T> {
 public:
  ...
  void SendMessage(v8::Isolate* isolate,
    ...
    if (!neutron_ipc_remote_) {
      thrower.ThrowError(kIPCMethodCalledAfterContextReleasedError);
      return;
    }
    blink::CloneableMessage message;
    if (!neutron::SerializeV8Value(isolate, arguments, &message)) {
      return;
    }
    neutron_ipc_remote_->Message(internal, channel, std::move(message));
  }
  ...
  void PostMessage(v8::Isolate* isolate,
    ...
    neutron_ipc_remote_->ReceivePostMessage(channel,
  ...
  void SendToHost(v8::Isolate* isolate,
    ...
    neutron_ipc_remote_->MessageHost(channel, std::move(message));
  }
  ...
  v8::Local<v8::Value> SendSync(v8::Isolate* isolate,
    ...
    neutron_ipc_remote_->MessageSync(internal, channel, std::move(message),
                                      &result);
    return neutron::DeserializeV8Value(isolate, result);
  }
 ...
 protected:
  mojo::AssociatedRemote<neutron::mojom::NeutronApiIPC> neutron_ipc_remote_;
};
...
class IPCRenderFrame : public IPCBase<IPCRenderFrame>,
 ...
 public:
  ...
  void OnDestruct() override { neutron_ipc_remote_.reset(); }
 ...
 public:
  explicit IPCServiceWorker(v8::Isolate* isolate) {
    ...
    neutron::ServiceWorkerData* service_worker_data =
        neutron::preload_realm::GetServiceWorkerData(
            isolate->GetCurrentContext());
    DCHECK(service_worker_data);
    service_worker_data->proxy()->GetRemoteAssociatedInterface(
        neutron_ipc_remote_.BindNewEndpointAndPassReceiver());
  }

  void WillStopCurrentWorkerThread() override { neutron_ipc_remote_.reset(); }
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_renderer_ipc, Initialize)
