...
v8::MaybeLocal<v8::Value> PassValueToOtherContextInner(
    ...
    int recursion_depth,
    BridgeErrorTarget error_target) {
  TRACE_EVENT0("neutron", "ContextBridge::PassValueToOtherContextInner");
  if (recursion_depth >= kMaxRecursion) {
    ...
    source_isolate->ThrowException(v8::Exception::TypeError(
        gin::StringToV8(source_isolate,
                        "Neutron contextBridge recursion depth exceeded.  "
                        "Nested objects "
...
v8::MaybeLocal<v8::Value> PassValueToOtherContext(
  ...
  TRACE_EVENT0("neutron", "ContextBridge::PassValueToOtherContext");
...
void ProxyFunctionWrapper(const v8::FunctionCallbackInfo<v8::Value>& info) {
  TRACE_EVENT0("neutron", "ContextBridge::ProxyFunctionWrapper");
...
// world ID. For service workers, Neutron only supports one isolated
...
v8::MaybeLocal<v8::Context> GetTargetContext(v8::Isolate* isolate,
  ...
  if (execution_context->IsWindow()) {
    ...
    maybe_target_context = neutron::preload_realm::GetInitiatorContext(
...
void ExposeAPIInWorld(v8::Isolate* isolate,
                      ...
                      const std::string& key,
                      v8::Local<v8::Value> api) {
  TRACE_EVENT2("neutron", "ContextBridge::ExposeAPIInWorld", "key", key,
               "worldId", world_id);
  v8::Local<v8::Context> source_context = isolate->GetCurrentContext();
...
void Initialize(v8::Local<v8::Object> exports,
  ...
  dict.SetMethod("executeInWorld", &neutron::api::ExecuteInWorld);
  dict.SetMethod("exposeAPIInWorld", &neutron::api::ExposeAPIInWorld);
  dict.SetMethod("_overrideGlobalValueFromIsolatedWorld",
                 &neutron::api::OverrideGlobalValueFromIsolatedWorld);
  dict.SetMethod("_overrideGlobalPropertyFromIsolatedWorld",
                 &neutron::api::OverrideGlobalPropertyFromIsolatedWorld);
#if DCHECK_IS_ON()
...
NODE_LINKED_BINDING_CONTEXT_AWARE(neutron_renderer_context_bridge, Initialize)
