...
NeutronRenderFrameObserver::NeutronRenderFrameObserver(
...
void NeutronRenderFrameObserver::DidClearWindowObject() {
...
void NeutronRenderFrameObserver::DidInstallConditionalFeatures(
  ...
  // This logic matches the EXPLAINED logic in neutron_renderer_client.cc
  // to avoid explaining it twice go check that implementation in
  // DidCreateScriptContext();
  bool is_main_world = neutron::is_main_world(world_id);
...
void NeutronRenderFrameObserver::WillReleaseScriptContext(
...
void NeutronRenderFrameObserver::OnDestruct() {
  delete this;
}

void NeutronRenderFrameObserver::DidMeaningfulLayout(
...
void NeutronRenderFrameObserver::CreateIsolatedWorldContext() {
  auto* frame = render_frame_->GetWebFrame();
  blink::WebIsolatedWorldInfo info;
  // This maps to the name shown in the context combo box in the Console tab
  // of the dev tools.
  info.human_readable_name =
      blink::WebString::FromUTF8("Neutron Isolated Context");
...
bool NeutronRenderFrameObserver::ShouldNotifyClient(int world_id) const {
