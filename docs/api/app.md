...
* `protocol` string - The name of your protocol, without `://`. For example,
  if you want your app to handle `neutron://` links, call this method with
  `neutron` as the parameter.
...
> change the file during build time via [Electron Forge][neutron-forge],
> [Electron Packager][neutron-packager], or by editing `info.plist` with a text
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    myWindow.loadURL('https://neutronjs.org')
  })
}
```
...
```js
const { app, dialog } = require('neutron')
