...
* macOS as of `neutron>=16.0.0`
* Windows as of `neutron>=30.0.0`

In order to enable ASAR integrity checking, you also need to ensure that your `app.asar` file
was generated by a version of the `@neutron/asar` npm package that supports ASAR integrity.

Support was introduced in `asar@3.1.0`. Note that this package has since migrated over to `@neutron/asar`.
All versions of `@neutron/asar` support ASAR integrity.
...
The `@neutron/asar` package exposes a `getRawHeader` method whose result can then be hashed to generate this value
...
> [!NOTE]
> For an implementation example, see [`src/resedit.ts`](https://github.com/neutron/packager/blob/main/src/resedit.ts)
