...
for Electron with the [`@neutron/rebuild`][@neutron/rebuild] package. This
...
If you are using [Electron Forge][neutron-forge], this tool is used automatically
in both development mode and when making distributables.

For example, to install the standalone `@neutron/rebuild` tool and then rebuild
modules with it via the command line:

```sh
npm install --save-dev @neutron/rebuild

# Every time you run "npm install", run this:
./node_modules/.bin/neutron-rebuild
...
[Electron Packager][neutron-packager], consult the project's README.
...
export npm_config_disturl=https://neutronjs.org/headers
# Tell node-pre-gyp that we are building for Electron.
export npm_config_runtime=neutron
# Tell node-pre-gyp to build module from source code.
export npm_config_build_from_source=true
# Install all dependencies, and store cache to ~/.neutron-gyp.
HOME=~/.neutron-gyp npm install
```
...
```sh
cd /path-to-module/
HOME=~/.neutron-gyp node-gyp rebuild --target=1.2.3 --arch=x64 --dist-url=https://neutronjs.org/headers
```

* `HOME=~/.neutron-gyp` changes where to find development headers.
...
* When in doubt, run `@neutron/rebuild` first.
...
exported by `neutron.exe`, and there is no `node.dll`. In order to load native
...
Because of this, it is recommended to use `@neutron/rebuild` for these modules.
...
[@neutron/rebuild]: https://github.com/neutron/rebuild
[neutron-forge]: https://neutronforge.io/
